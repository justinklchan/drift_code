...
void bqRecorderCallback_old(SLAndroidSimpleBufferQueueItf bq, void *context)
{
...
Java_com_example_nativeaudio_NativeAudio_getTimestamps(JNIEnv* env, jclass clazz,
                                                       jint recordTime, jint fs, jint bufferSize_mic) {
    jlong out[(recordTime*FS)/bufferSize_mic];
    int i = 0;
    for(i = 0; i < (recordTime*FS)/bufferSize_mic; i++){
        out[i] = cxt2->mic_ts[i]-cxt2->mic_ts[0];
    }
    jlongArray result;
    result = (*env)->NewLongArray(env,(recordTime*FS)/bufferSize_mic);
    (*env)->SetLongArrayRegion(env, result, 0, (recordTime*FS)/bufferSize_mic, out);
...
//    }
    ...
    result = (*recorderBufferQueue)->RegisterCallback(recorderBufferQueue, bqRecorderCallback, cxt2);
    assert(SL_RESULT_SUCCESS == result);
